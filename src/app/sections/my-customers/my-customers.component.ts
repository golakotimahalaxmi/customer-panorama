...
export class MyCustomersComponent implements OnInit, OnDestroy {
  ...
  openModalfromsidenav() {
    this.unSubscribe = this.dashboardService.openModalFromLeftNav.subscribe(
      (modalName) => {
  ...
  getAllHedings() {
    if (
      this.dashboardService.storeAllheadings === undefined ||
      this.dashboardService.storeAllheadings === null
    ) {
