...
export class SideNavigationComponent implements OnInit {
  ...
  sideNavitems = [
     ...
     },
     {
      ...
      "bottom":"yes",
      "next":"yes",
      "toolTip": "Coming Soon! View all contacts within your book of customers. Filter by customers, top purchasers, etc",
      "link":"no",
      "last":'no'
    },
    {
      ...
      "bottom":"yes",
      "next":"no",
      "toolTip":"Reports to help track important information like Conversion Rates, Goal Tracking, etc",
      "link":"yes",
      "last":'no'
    },
    {
  ...
  constructor(private router: Router, private dashboard: DashboardService, private modelService: ModalService) {
    this.headings = new Headings();
    if(this.router.url.split("/")[1] === 'customer')
    {
      this.link_highlight=0;
      this.selectedNavItem(this.sideNavitems[this.link_highlight],this.link_highlight);
    }
    ...
    if(item.link === 'yes')
     this.sideNavitems.forEach((val, index) => {
       if(val.itemName === item.itemName) {
         val.border = 'yes';
         if(index>0)
          this.sideNavitems[index-1].bottom = 'no';
       } else {
         val.border = 'no';
         val.bottom = 'yes';
       }
