...
import { MycustomersDetails, Customerevents, Customernewsalerts, Partnerslist, Customercontacts, Firmographics, Contactinfluence, Customerfinanical, Conversion, Recommandation, SimilarCompany, AccountManager_locations, Savefeedback, Viewcomments } from '../classesList/customer';
...
export class CustomerService {
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 1. sort
    let customers = this.sort(this.customerFullDetails, sortColumn, sortDirection);

    // 2. filter
    customers = customers.filter(customersDetail => this.searchingItems(customersDetail, searchTerm));
...
searchingItems(customersDetail: MycustomersDetails, term: string) {
  ...
  return customersDetail.customer_name.toLowerCase().includes(term.toLowerCase())
...
sort(customers: MycustomersDetails[], column: string, direction: string): MycustomersDetails[] {
  if (direction === '') {
    return customers;  
  } else {
...
showParticularStackholderData(){
  ...
  changeDataBasedOnId(variable: string) {
   ...
   getFinanicalData(): Observable<Customerfinanical[]> {
    return this.http.get<Customerfinanical[]>('assets/data/'+this.customerType+'/customer_financials.json');
  }

  findFundingJson(): Observable<Customernewsalerts[]> {
    return this.http.get<Customernewsalerts[]>('assets/data/'+this.customerType+'/funding.json');
  }

  // method for fecting service data of conversion probability
  getConversionData(): Observable<Conversion[]>{
    return this.http.get<Conversion[]>('assets/data/'+this.customerType+'/customer_conversion.json');
  }
  ...
  getRecommandation(): Observable<Recommandation[]> {
      return this.http.get<Recommandation[]>('assets/data/'+this.customerType+'/recommendations.json')
  }

   // method for getting finanical data for customer in identity component
  getAccountManager_locations(): Observable<AccountManager_locations[]> {
    return this.http.get<AccountManager_locations[]>('assets/data/'+this.customerType+'/international_managers.json');
  }

  setFeedback(feedback): Observable<Savefeedback> {
    return this.http.post<Savefeedback>('http://cprofile.corp.cdw.com:8182/setFeedback', feedback);
  }

  getFeedback() : Observable<Viewcomments[]> {
    return this.http.get<Viewcomments[]>('http://cprofile.corp.cdw.com:8182/getFeedback');
  }
}
